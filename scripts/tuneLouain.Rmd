...
param_grid = rbind(expand.grid("nn" = seq(1, 150, length.out = 40),
                   ...
                   expand.grid("nn" = "default",
                               "had" = c(TRUE, FALSE)),
                   expand.grid("nn" = nclass.Sturges(1:nrow(CBCL.X)),
                               "had" = c(FALSE)))
...
resFrame = data.frame(
  stringsAsFactors = F,
  best.k = sapply(louv_tune, function(x) x$optimal.k),
  modularity = sapply(louv_tune, function(x) x$modularity),
  nbNeighbors = as.numeric(replace(param_grid[,1], param_grid[, 1] == "default", "13")),
  HadamardMethod = param_grid[,2]
) %>%
  mutate_at("nbNeighbors", ~as.numeric(replace(., . == "default", nclass.Sturges(1:nrow(CBCL.X))))) %>%
  arrange(nbNeighbors)
p<-ggplot(resFrame[-1,]) +
  ...
  geom_hline(aes(yintercept = sort(unique(resFrame$best.k))[which.max(table(resFrame$best.k))])) +
  theme(legend.position = "left") +
  ggtitle(paste0("Most often cluster size: ", sort(unique(resFrame$best.k))[which.max(table(resFrame$best.k))]))
