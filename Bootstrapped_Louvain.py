...
# Louvain Community Detection Script 
...
df = pd.read_csv('C:/Users/jacob.derosa/Desktop/Scripts/Functional_Cors/fc_cbcl.csv')
df = df.rename(columns={'Unnamed: 0': 'Key'})
subset = ["R_X.3..8.","R_X.3..9.","R_X.7..13.","R_X.1..12.","R_X.1..6.","R_X.10..0.","R_X.3..12.","R_X.7..12.","R_X.4..11."] #Variables you want to cluster on 

y = np.array(df['Key']) #put subject ID into a list that will be matched with bootsrapped indicices. This important because this step will be used later on to match ID to cluster assignment. 

n_straps = 1000 #number of bootsraps 
n = df.shape[0]
...
allcsvs = {}
for i in range(n_straps):
    df2 = {'URSI':bootstrap_split_subids[i][0],'cluster':bootstrap_split_communities[i][0]}
    allcsvs[i] = pd.DataFrame(df2)
    allcsvs[i].to_csv('C:/Users/jacob.derosa/Desktop/Scripts/Functional_Cors/FC_Cors_Data/cluster_fc_csv_%s.csv' % i)
